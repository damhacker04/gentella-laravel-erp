{{-- Shared Form for Sales Order Create & Edit with Dynamic Line Items --}}
@php
...
<div class="row">
    <div class="col-md-4">
        <x-select name="customer_id" label="Pelanggan" :options="$customers"
                  option-value="id" option-label="name"
                  :selected="$salesOrder->customer_id ?? ''"
                  required searchable placeholder="Pilih Pelanggan..." />
    </div>
    <div class="col-md-3">
        <x-date name="order_date" label="Tanggal Order"
                :value="$salesOrder->order_date ?? date('Y-m-d')" required />
    </div>
    <div class="col-md-3">
        @if($isEdit)
        <div class="mb-3">
...
<div class="row">
    <div class="col-md-12">
        <x-textarea name="notes" label="Catatan" :value="$salesOrder->notes ?? ''"
